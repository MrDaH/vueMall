...
//1.首先webpack发现我们并没有通过命令行的形式，给它指定入口和出口
//2.webpack就会项目的根目录中，查找一个叫做'webpack.config.js'文件
//3.当找到配置文件后，webpack会去解析执行这个配置文件，解析后会得到配置文件中导出的配置对象
//4.当webpack拿到配置对象后，就拿到了入口和出口，然后就进行打包构建


//使用webpack-dev-server这个工具来实现自动打包编译的功能
//1.运行npm i webpack-dev-server这个工具来实现自动打包编译的功能 -D 把这个工具安装到本地开发依赖
//2.由于我们是局部安装的工具，没办法直接在powershell终端中使用（只有全局-g安装的工具才可以）
